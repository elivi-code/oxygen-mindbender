...
const getColumnMeta = ({ type = defaultType, pinned }: ColumnDef) => ({
...
const getPinnedColumns = (columnDefs: PinnedColumnDef[]): PinnedColumns => {
  ...
  let currentOffset = 0;
  const pinnedColumns: PinnedColumns = {};
  ...
  pinnedColumns[lastPinnedColumnId as string].isLastPinnedColumn = true;

  return pinnedColumns;
};

const adaptColumnDefinition = <TData>(
  columnDef: ReactTableColumnDef<TData, string>,
) => {
