...
const BodyRowComponent = <TData,>({
  ...
  return (
    <BodyRow
      ref={rowRef}
      {...getTestAttributes(`${testId}-row-${row.id}`)}
      ...
      style={style}
    >
      {rowStatus && <WarningIndicator rowStatusType={rowStatus} />}
      <RowActions
        show={showActionsOnHover || showActionsOnFocus}
        actions={actionsMap?.[row.id]}
        rowActionsPortalTarget={rowActionsPortalTarget}
        isActive={activeRowId === row.id}
        testId={`${testId}-rowActions`}
      />
      {row.getVisibleCells().map(cell => {
        ...
        return (
          <Cell
            {...getTestAttributes(`${testId}-rowCell-${cell.id}`)}
            as="td"
            align={meta.align}
            width={size}
            flexible={meta.flexible}
            pinned={pinnedColumns?.[cell.column.id]}
            showPinnedBorder={showPinnedBorder}
            key={cell.id}
            hasError={Boolean(errorMap[row.id])}
          >
