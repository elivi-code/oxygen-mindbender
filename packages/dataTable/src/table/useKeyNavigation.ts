...
const isNavigationKey = (key: string) =>
  ['Tab', 'ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight'].includes(key);

export const useKeyNavigation = ({ blurOnTab }: Props) => {
  const isKeyboardNavigation = useRef(false);
  const isTableBodyFocused = useRef<boolean>(false);
  const hiddenRowRef = useRef<HTMLTableRowElement>(null);
  const firstRowRef = useRef<HTMLTableRowElement>(null);

  useEffect(() => {
    const documentOnKeyDown = (e: globalThis.KeyboardEvent) => {
