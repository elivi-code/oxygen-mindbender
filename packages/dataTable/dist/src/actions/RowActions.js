...
const StyledRowActions = styled.div `
  ...
  padding: ${({ theme }) => `0 ${StyledRowActionsPaddingRight}px 0 ${theme.spacing06}`};
...
`;
const RowActions = ({ actions, show, rowActionsPortalTarget, isActive, testId = 'rowActions', }) => {
    if (!actions || !show) {
        return null;
    }
    ...
    return (React.createElement(RowActionsContainer, null,
        React.createElement(StyledRowActions, { isActive: isActive },
            inlineActions.map(action => (React.createElement(InlineRowAction, { testId: action.testId, label: action.label, onClick: event => {
                    action.onClick();
                    event?.stopPropagation();
                }, disabled: action.disabled, icon: action.icon, key: action.key }))),
            rowActionsPortalTarget && (React.createElement(MoreRowActions, { actions: moreActions, rowActionsPortalTarget: rowActionsPortalTarget, testId: `${testId}-moreRowActions` })))));
};
export { RowActions, StyledRowActions };
