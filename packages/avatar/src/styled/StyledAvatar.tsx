...
const getStatusBorder = (
  ...
  const borderSize = (
    [avatarSize.size3xsmall, avatarSize.size2xsmall] as AvatarSize[]
...
const StyledAvatar = styled.div<{
  ...
  ${({ hasBorder, theme }) =>
    hasBorder && `border: 2px solid ${theme.borderColor};`}
  display: flex;
  position: relative;
  align-items: center;
...
const EditOverlay = styled.div<{
  ...
  height: ${({ theme, size }) =>
    `calc(0.3 * ${
      theme[`size${size[0].toUpperCase() + size.slice(1)}` as keyof AvatarTheme]
    })`};
  position: absolute;
