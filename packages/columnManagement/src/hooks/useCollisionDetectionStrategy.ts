...
export function useCollisionDetectionStrategy(
  ...
  lastOverId: React.MutableRefObject<UniqueIdentifier | undefined>,
) {
  /**
  ...
  const collisionDetectionStrategy: CollisionDetection = useCallback(
    args => {
      ...
      const containerItems = items[overId as ColumnsType];

      // If a container is matched
      if (containerItems.length > 0) {
        if (
          overId === ColumnsType.CurrentColumns &&
        ...
        overId = closestCenter({
          ...
          droppableContainers: args.droppableContainers.filter(
            container =>
              container.id !== overId &&
              containerItems.some(
                (item: { id: UniqueIdentifier }) => item.id === container.id,
              ),
          ),
        })[0]?.id;
      }
