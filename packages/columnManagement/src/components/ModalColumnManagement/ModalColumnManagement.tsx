...
import { ModalColumnManagementProps } from '../../types/ModalColumnManagement';
...
export const ModalColumnManagement = ({
  availableColumns,
  currentColumns,
  defaultColumns,
  onClose,
  onReset,
  onSave,
  modalTestId = 'MODAL_COLUMN_MANAGEMENT',
  testId,
  translations,
  telemetryProps,
}: ModalColumnManagementProps) => {
  const [updatedCurrentItems, setUpdatedCurrentItems] =
    useState<ColumnInterface[]>(currentColumns);
  const [isResetBtnEnabled, setIsResetBtnEnabled] = useState<boolean>(true);
  const [isCloseBtnEnabled, setIsCloseBtnEnabled] = useState<boolean>(false);
  const [isResetBtnTriggered, setIsResetBtnTriggered] =
  ...
  const handleUpdate = (items: ColumnInterface[]) => {
  ...
  const handleSave = () => {
    if (telemetryProps && telemetryCallback) {
      telemetryCallback('edit_columns_saved', {
        ...
        availableColumnsNumber: differenceBy(
          availableColumns,
          updatedCurrentItems,
          'name',
        ).length,
  ...
  return (
    <Modal testId={modalTestId} size={MODAL_SIZE.BIG} onClose={handleCancel}>
      <StyledModalHeader
        title={
          translations?.modalHeaderTitle ||
          DEFAULT_TRANSLATIONS.modalHeaderTitle
        }
      ...
      <ModalContent>
        ...
        <ModalColumnManagementContext.Provider
          value={ModalColumnManagementContextValue}
        >
          <ColumnManagement
      ...
      <ModalFooter>
        <Button
          isDisabled={!isResetBtnEnabled}
          onClick={handleReset}
          variant={buttonVariant.text}
          testId={`${modalTestId}_${DEFAULT_TRANSLATIONS.modalReset}`}
        >
          {translations?.modalReset || DEFAULT_TRANSLATIONS.modalReset}
        </Button>
        <Button
          onClick={handleCancel}
          variant={buttonVariant.secondary}
          testId={`${modalTestId}_${DEFAULT_TRANSLATIONS.modalCancel}`}
        >
          {translations?.modalCancel || DEFAULT_TRANSLATIONS.modalCancel}
        </Button>
        <Button
          isDisabled={!isCloseBtnEnabled}
          onClick={handleSave}
          testId={`${modalTestId}_${DEFAULT_TRANSLATIONS.modalSave}`}
        >
          {translations?.modalSave || DEFAULT_TRANSLATIONS.modalSave}
        </Button>
      </ModalFooter>
    </Modal>
  );
