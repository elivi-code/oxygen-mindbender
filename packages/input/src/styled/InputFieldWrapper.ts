...
const InputFieldWrapper = styled.div<InputFieldWrapperProps>`
  display: flex;
  flex: 1;
  border-width: ${({ theme, withPrefix, withSuffix, focused, isReadOnly }) =>
    getInputBorderWidth({
      theme,
      withSuffix,
      withPrefix,
  ...
  border-radius: ${({ theme, withSuffix, withPrefix }) =>
  ...
  margin: ${({ theme, isReadOnly }) =>
    isReadOnly ? `0 0 0 -${theme.borderWidth}` : 0};
  padding: ${getPadding};
