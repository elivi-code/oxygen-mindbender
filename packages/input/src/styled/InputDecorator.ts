...
import getBorderColor, { GetBorderColorProps } from './utils/getBorderColor';
...
const getDecoratorBorderColor = ({
  isRight,
  theme,
  ...props
}: { isRight: boolean } & GetBorderColorProps) => {
...
const InputDecorator = styled.div<InputDecoratorProps>`
  align-items: center;
  background-color: ${({ theme, isReadOnly }) =>
    getDecoratorBackgroundColor({ theme, isReadOnly })};
  border: ${({ theme, isReadOnly }) =>
    getDecoratorBorder({ theme, isReadOnly })};
  border-right-color: ${props =>
    getDecoratorBorderColor({ ...props, isRight: Boolean(props.isPrefix) })};
  border-left-color: ${props =>
    getDecoratorBorderColor({ ...props, isRight: !props.isPrefix })};
  ...
  width: ${({ theme, autoWidth }) =>
    autoWidth ? 'auto' : theme.widthSemanticBlock};
