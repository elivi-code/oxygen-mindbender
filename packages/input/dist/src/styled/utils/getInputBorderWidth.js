const getInputBorderWidth = ({ theme, withSuffix, withPrefix, focused, isReadOnly, }) => {
    let borderTop;
    let borderRight;
    let borderBottom;
    let borderLeft;
    const borderProp = isReadOnly && !focused ? 'borderWidthReadOnly' : 'borderWidth';
    const borderValues = borderProp && theme[borderProp] ? theme[borderProp].split(' ') : '';
