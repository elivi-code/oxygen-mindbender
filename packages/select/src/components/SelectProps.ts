...
export interface LabelPropsExtended
  extends Pick<
    LabelProps,
    | 'action'
    | 'actionLabel'
    ...
    | 'otherActionProps'
  > {
  labelValue?: string; // forwarded to Label.value
  shouldWrapLabel?: boolean; // forwarded to Label.shouldWrapText
}
...
> = IsAsync extends true
  ? Omit<AsyncProps<Option, IsMulti, Group>, keyof Props>
  : unknown;
...
export type SelectProps<
  ...
  IsMulti extends boolean,
  Group extends GroupBase<Option>,
> = Omit<Props<Option, IsMulti, Group>, 'theme' | 'loadingMessage'> &
  PickAsyncProps<Option, IsAsync, IsMulti, Group> &
  LabelPropsExtended &
  Partial<CuratedCustomSelectProps> & // re-adding CustomSelectProps as optional
  OwnProps<IsAsync>;
