...
const SelectContainer = styled.div<{ hasLabel: boolean; isDisabled: boolean }>`
...
const CloseNovoIcon = styled(CloseCircleIcon)`
  fill: ${({ theme }) => (theme as any).icon01};
`;

CloseNovoIcon.displayName = 'CloseNovoIcon';

const CloseIcon = styled(CloseCircleIcon)<{ isFocused: boolean }>`
  &:hover {
    fill: ${({ isFocused, theme }) =>
      isFocused ? theme.clearIconFocusedHoverColor : theme.clearIconHoverColor};
  }
`;
...
const OptionCheckboxLayer = styled.div<{ isDisabled: boolean }>`
  position: absolute;
  left: 0;
  top: 0;
  width: 100%;
...
const ValueWrapper = styled.div<
  Pick<CustomSelectProps, 'size' | 'multipleSelectMaxRows'>
>`
  align-items: center;
  display: flex;
  flex: 1;
