...
function getCustomStyles({ backgroundColor, backgroundColorDisabled, backgroundColorHover, borderColorHover, borderColor, borderColorDisabled, borderColorError, borderColorFocus, borderWidth, controlFocusBoxShadow, dropdownIndicatorColor, dropdownIndicatorColorDisabled, fontFamily, headingBorderColor, headingBorderSize, headingBorderStyle, headingMargin, headingPadding, headingTextColor, headingTextFontSize, headingTextFontWeight, indicatorsContainerPaddingHorizontal, inputBoxSizeDefault, inputBoxSizeLarge, inputBoxSizeSmall, inputColor, menuBackgroundColor, menuBorderColor, menuBorderSize, menuBorderStyle, menuBoxShadowColor, menuBoxShadowSize, menuZindex, noOptionTextColor, noOptionTextFontSize, optionActiveBackgroundColor, optionHoveredBackgroundColor, optionPaddingHorizontal, optionPaddingVertical, optionSelectedBackground, optionTextColor, optionTextFontSize, optionTextFontWeightSelected, optionTextSelectedColor, placeholderTextColor, placeholderTextColorDisabled, selectedOptionTextColor, selectedOptionTextColorDisabled, valueContainerPadding, }) {
    const getOptionTextStyles = (isSelected) => {
        if (isSelected) {
            return {
    ...
    const getBorderColor = ({ isDisabled, isFocused, hasError, }) => {
        if (isDisabled) {
    ...
    const getHoverBorderColor = ({ isDisabled, isFocused, hasError, }) => {
        if (isDisabled) {
            return borderColorDisabled;
        }
        if (hasError) {
     ...
     * */
    const getIndicatorsContainerPadding = (size, iconSet) => {
        if (size === 'large') {
            return '10px';
        }
        if (size === 'small') {
    ...
    return {
        control: (providedStyle, { isDisabled, isFocused, selectProps: { size, hasError, isReadOnly } }) => ({
            ...providedStyle,
            ':hover': {
            ...
            background: isDisabled || isReadOnly ? backgroundColorDisabled : backgroundColor,
        ...
        dropdownIndicator: (providedStyle, { isDisabled, selectProps: { isReadOnly } }) => ({
            ...providedStyle,
            color: isDisabled || isReadOnly
        ...
        indicatorsContainer: (providedStyle, { selectProps: { size }, theme: { iconSet } }) => ({
            ...providedStyle,
            alignSelf: 'center',
            padding: `${getIndicatorsContainerPadding(size, iconSet)} ${indicatorsContainerPaddingHorizontal}`,
        }),
        ...
        option: (providedStyle, state) => {
            const { isFocused, isSelected, isDisabled, selectProps: { isReadOnly }, } = state;
            return {
                ...providedStyle,
                fontFamily,
        ...
        placeholder: (providedStyle, { isDisabled, selectProps: { isSearchable, isReadOnly } }) => ({
            ...providedStyle,
            fontSize: optionTextFontSize,
            color: isDisabled || isReadOnly
        ...
        singleValue: (providedStyle, { isDisabled, selectProps: { isSearchable, isReadOnly } }) => ({
            ...providedStyle,
            color: isDisabled || isReadOnly
