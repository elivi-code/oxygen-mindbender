...
import { Props, InternalPropsWithTheme } from './styledProps';

const getKnobTranslation = ({
  theme,
  isChecked,
  isIndeterminate,
}: Pick<InternalPropsWithTheme, 'theme' | 'isChecked' | 'isIndeterminate'>) => {
...
const ToggleButtonIcon = styled.span<Props>`
  display: flex;
  background: ${({ theme, isChecked, isDisabled, isIndeterminate }) =>
    getBackgroundColor({
      theme,
      isChecked,
      isDisabled,
  ...
  opacity: ${({ isDisabled, theme }) =>
    theme.iconSet !== 'novo' && isDisabled ? '0.5' : '1'};
  &::after {
    content: '';
    width: ${({ theme }) => theme.knobSize};
    height: ${({ theme }) => theme.knobSize};
    background: ${({ theme, isDisabled }) =>
      isDisabled ? theme.knobColorDisabled : theme.knobColor};
    visibility: visible;
    border-radius: 100%;
    ...
    ${({ theme, isChecked, isIndeterminate }) =>
      getKnobTranslation({ theme, isChecked, isIndeterminate })}
  }
`;
