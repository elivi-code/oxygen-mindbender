...
import { arrow, autoUpdate, flip, FloatingPortal, limitShift, offset as floatingUiOffset, safePolygon, shift, useClick, useDismiss, useFloating, useFocus, useHover, useInteractions, useRole, useMergeRefs, } from '@floating-ui/react';
import StaticTooltip from '@8x8/oxygen-static-tooltip';
import { getTestAttributes, withTheme, withDeprecation, } from '@8x8/oxygen-config';
import { useId } from '@8x8/oxygen-utils';
import { registerEventHandlers, unregisterEventHandlers, parseModifiers, } from '../utils';
import Arrow from './Arrow';
import { showOn as showOnValues, orientation as orientationValues, } from '../types';
const side = {
    top: 'bottom',
    right: 'left',
    bottom: 'top',
...
const Tooltip = ({ children, customCloseHandlers = [], delay = 0, enableArrow, includeWrapper = true, offset = 7, modifiers, orientation = orientationValues.top, renderContainer, renderContainerId, showOn = showOnValues.hover, disableInteractive = false, testId = 'TOOLTIP', theme, title, triggerRef, forwardedRef, ...rest }) => {
    const [open, setOpen] = useState(false);
    // @deprecated modifiers — use offset instead directly
    const computedOffset = parseModifiers(modifiers) || offset;
    const handlers = React.useCallback(() => customCloseHandlers.map(handler => ({
        ...handler,
    ...
    const { x, y, refs, strategy, context, placement, middlewareData: { arrow: { x: arrowX, y: arrowY } = {} }, } = useFloating({
        open,
    ...
    const staticSide = side[placement.split('-')[0]];
    const renderWithoutWrapper = () => React.Children.map(children || null, (child, index) => {
        // only use the first child as target
        if (index === 0 && React.isValidElement(child)) {
            return cloneElement(child, getReferenceProps({
    ...
    const renderWithWrapper = () => (React.createElement("div", { ...getReferenceProps({
            ref: useMergeRefs([refs.setReference, forwardedRef]),
    ...
    return (React.createElement(React.Fragment, null,
        ...
        React.createElement(FloatingPortal, { ...floatingPortalProps }, open && (React.createElement("div", { ref: refs.setFloating, style: {
                position: strategy,
                top: y ?? 0,
                left: x ?? 0,
                width: 'max-content',
            }, ...getFloatingProps() },
            React.createElement(StaticTooltip, { theme: theme, testId: `STATIC_TOOLTIP_FOR_${testId}`, renderArrow: () => (enableArrow ?? theme.enableArrow) && (React.createElement(Arrow, { ref: arrowRef, ...getTestAttributes(`tooltip_arrow_${testId}`), style: {
                        left: arrowX != null ? `${arrowX}px` : '',
                        top: arrowY != null ? `${arrowY}px` : '',
                        [staticSide]: theme.arrowMarginOffset,
                    } })), ...rest }, title))))));
};
export default withDeprecation(withTheme(Tooltip, 'staticTooltip'), `Tooltip deprecated props: "modifiers", "customCloseHandlers"`);
