...
const DropdownButton = ({ theme, customCloseHandlers, forwardedRef, shouldCloseOnInsideClick, isDefaultOpen, indicatorIcon, children, buttonText, idealDirection, isDisabled, onOpen, onClose, invertIconPosition, renderInPortal, ...otherProps }) => {
    const buttonRef = forwardedRef || useRef();
    ...
    const renderDropdownIndicator = () => {
        ...
        if (indicatorIcon === undefined) {
            return theme.isClassic ? (React.createElement(DropdownIndicator, { ...baseProps })) : (React.createElement(DropdownIcon, { ...baseProps }));
        }
        // @todo properly mark as deprecated
        if (indicatorIcon) {
            return React.createElement(DropdownIcon, { ...baseProps, as: indicatorIcon });
        }
        return null;
    };
    const renderDropdown = () => (React.createElement(Popper, { placement: idealDirection, positionFixed: true, innerRef: setDropdownRef }, ({ ref, style }) => (React.createElement(DropdownWrapper, { style: style, ref: ref, onClick: onDropdownClick, onKeyDown: onDropdownKeyDown },
        React.createElement(ScrollWrapper, null, children)))));
    return (React.createElement(ThemeProvider, { theme: theme },
        React.createElement(Manager, null,
            React.createElement(Reference, null, ({ ref }) => (React.createElement(StyledButton, { variant: buttonVariant.secondary, isDisabled: isDisabled, 
                // @todo test ref forwarding
                ref: (node) => {
                    ref(node);
                    setButtonRef(node);
                }, onClick: onButtonClick, onKeyDown: onButtonKeyDown, theme: theme.button, invertItems: invertIconPosition, ...otherProps, "aria-expanded": isOpen, "aria-haspopup": true },
                buttonText,
