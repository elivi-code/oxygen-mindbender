...
import { KeyboardEventType } from '../../functions/keyboardEvents';
 ...
 * @returns { void }
 */
const useGlobalActionKeyHandler = ({
  actionKeyHandler,
  windowContext,
  fireEvents = [EventKey.Enter, EventKey.Space],
}: HookProps): void => {
  const handleKeyPress = (event: KeyboardEventType): void => {
    if (fireEvents.includes(event.key as EventKey)) {
  ...
  useEffect(() => {
    (windowContext || window).document.addEventListener(
      'keydown',
      handleKeyPress,
    );
