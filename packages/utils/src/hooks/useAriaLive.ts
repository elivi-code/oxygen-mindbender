...
const messageTimeouts = {
  polite: null as NodeJS.Timeout | null,
  assertive: null as NodeJS.Timeout | null,
};

/**
 * Get or create the DOM container for the specified type
 */
function getOrCreateContainer(type: 'polite' | 'assertive'): HTMLElement {
 ...
 */
function announceMessage(message: string, type: 'polite' | 'assertive'): void {
...
export function cleanup(): void {
  ...
  Object.keys(messageTimeouts).forEach(type => {
    const key = type as keyof typeof messageTimeouts;
...
export default function useAriaLive(): UseAriaLiveReturn {
  const announce: UseAriaLiveReturn['announce'] = useCallback(
    (message: string, messageOptions: { assertive?: boolean } = {}) => {
