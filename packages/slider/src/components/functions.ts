 ...
 */
export function setOffsetsAndWidths(
  ...
  setHalfProgressWidth?: (width: string) => void,
) {
  ...
  setAfterThumbTrackWidth(
    `calc(${100 - maxElemValue}% - ${maxElemKnobPercent}px)`,
  );
  setBeforeThumbTrackWidth(`calc(${minElemValue}% - ${minElemKnobPercent}px)`);

  if (setFirstHalfProgressLeft) setFirstHalfProgressLeft(`${minElemValue}%`);

  if (setSecondHalfProgressLeft) setSecondHalfProgressLeft(`${avg}%`);

  if (setHalfProgressWidth) setHalfProgressWidth(`${maxElemValue - avg}%`);
}
