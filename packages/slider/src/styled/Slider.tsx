...
export const SliderContainer = styled.div<Pick<SliderProps, 'isDisabled'>>`
  position: relative;
  display: flex;
  align-items: center;
...
export const TrackAfterMax = styled.div.attrs<{
  width: string;
}>(({ width }) => ({
  style: {
    width,
  ...
  Pick<SliderProps, 'isDisabled' | 'theme'> & {
    width: string;
...
export const TrackBeforeMin = styled.div.attrs<{
  width: string;
}>(({ width }) => ({
  style: {
    width,
...
>`
  background-color: ${({
    ...
    if (isDisabled) return theme.sliderActiveTrackBackgroundColorDisabled;

    if (isMultiple) return theme.sliderTrackBackgroundColor;

    if (isActive || isFocused) return theme.sliderKnobColorActive;
...
export const HalfProgress = styled.div.attrs<{
  left: string;
  width: string;
}>(({ left, width }) => ({
  style: {
...
export const InputForDraggableRange = styled.input.attrs<{
  left: string;
  width: string;
}>(({ left, width }) => ({
  type: 'range',
...
}))<{ left: string; width: string; expandTrackAreaBy: string }>`
  position: absolute;
  height: 5px;
  cursor: pointer;
  opacity: 0;
...
export const SliderInput = styled.input.attrs(() => ({
  ...
  Pick<
    SliderProps,
    'theme' | 'isDisabled' | 'isMultiple' | 'expandTrackAreaBy'
  > & {
    isActive?: boolean;
    isHovered?: boolean;
    isFocused?: boolean;
