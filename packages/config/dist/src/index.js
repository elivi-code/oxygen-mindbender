...
let logger = console.warn; // eslint-disable-line no-console
 ...
 */
function withTheme(WrappedComponent, configProp) {
    const WithTheme = React.forwardRef((props, ref) => {
        ...
        if (!context) {
            ...
            const componentTheme = enableNeo
                ? // maps to ${componentName}NeoLight or ${componentName}NeoDark from constants
                    `${configProp}${neoThemeMapper[themeName]}`
                : // maps to ${componentName}Novo or ${componentName}NovoDark from constants
                    `${configProp}${themeMapper[themeName]}`;
            computedTheme = {
        ...
        const refs = WrappedComponent.componentType === COMPONENT_TYPE_HOC
            ? { ref }
            : { forwardedRef: ref };
        // eslint-disable-next-line @typescript-eslint/ban-ts-comment
        // @ts-ignore
        return React.createElement(WrappedComponent, { ...rest, theme: componentTheme, ...refs });
    });
...
function withDeprecation(WrappedComponent, deprecationMessage) {
    const WithDeprecation = React.forwardRef((props, ref) => {
        const refs = WrappedComponent.componentType === COMPONENT_TYPE_HOC
            ? { ref }
            : { forwardedRef: ref };
        return React.createElement(WrappedComponent, { ...props, ...refs });
    });
    ...
    if (isReactDevMode()) {
        const message = deprecationMessage ||
            `Component "${WrappedComponent.displayName || WrappedComponent.name || ''}" is deprecated`;
        logger(`${message}; Please check Oxygen changelog for more info: https://github.com/8x8/oxygen/blob/master/CHANGELOG.md`);
    }
