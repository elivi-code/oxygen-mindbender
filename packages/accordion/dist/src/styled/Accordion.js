...
const InteractiveHeader = styled.button `
  ...
  ${({ theme, isNotClickable }) => !isNotClickable &&
    `&:hover {
        background-color: ${theme.ui05};
     }`}
`;
// Non-interactive header (native div element)
const NonInteractiveHeader = styled.div `
  ${headerBaseStyles}
...
const ContentContainer = styled.div `
  ...
  ${({ hasFixedHeight, height, isExpanded }) => hasFixedHeight
    ? `
...
const ContentAfterTitle = styled.div `
  ...
  gap: 4px;
`;
export { AccordionWrapper, Content, ContentAfterTitle, ContentContainer, ContentWrapper, InteractiveHeader, NonInteractiveHeader, IconContainer, SecondaryText, Title, TitleText, };
