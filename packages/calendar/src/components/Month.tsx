...
import Day from './Day';
import { DateOptions, DateRange } from '../types';
import { InternalCalendarProps } from './CalendarProps';

function getMonthDisplayRange(date: Date, dateOptions?: DateOptions) {
...
export type MonthProps = OwnProps &
  Pick<
...
type MonthDefaultProps = Required<
  Pick<
    MonthProps,
    | 'testId'
    | 'dateOptions'
...
class Month extends PureComponent<MonthProps> {
  ...
  renderMonthName = () => {
    const { testId, month, dateOptions, monthDisplayFormat, locale } =
      this.props;
    ...
    return (
      <StyledMonth.MonthName
        {...getTestAttributes(`${testId}_MONTH_NAME`)}
        role="row"
      >
        <div role="cell">
          {format(month, monthDisplayFormat as string, formatOptions)}
        </div>
      </StyledMonth.MonthName>
    );
  };
  ...
  renderWeekDays = () => {
    ...
    return (
      <StyledMonth.WeekDays
        {...getTestAttributes(`${testId}_WEEK_DAYS`)}
        role="row"
      >
        {eachDayOfInterval({
          ...
          <StyledMonth.WeekDay key={day.toString()} role="columnheader">
            {format(day, weekDayDisplayFormat as string, formatOptions)}
          </StyledMonth.WeekDay>
        ))}
      </StyledMonth.WeekDays>
    );
  ...
  render() {
    ...
    return (
      <ThemeProvider theme={theme}>
        <StyledMonth.MonthGrid {...getTestAttributes(monthTestId)} role="grid">
          ...
          {eachWeekOfInterval(
            ...
            const weekTestId = `${testId}_WEEK_${getWeek(
              week,
              dateOptions,
            )}_${month.getFullYear()}`;
                ...
                {eachDayOfInterval({
                  ...
                  return (
                    <Day
                      ...
                      key={day.toString()}
                      day={day}
                      testId={`${testId}_MONTH_${day.getMonth()}_${day.getFullYear()}_DAY_${day.getDate()}`}
                      ...
                      isDisabled={isOutsideMinMax || dayDisabled}
                      isToday={isSameDay(day, now)}
                      isWeekend={isWeekend(day)}
                      isStartOfWeek={isSameDay(
                        day,
                        startOfWeek(day, dateOptions),
                      )}
