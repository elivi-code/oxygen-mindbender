...
function getTodayLineColor({
  isPassive,
  isSelected,
  isEdge,
  theme,
}: DayCellProps & { theme: CalendarTheme }) {
  if (isPassive) {
...
function getSelectionRadius({ isStartEdge, isEndEdge }: SelectionRangeProps) {
  let radius = '';
...
function getPreviewBorderStyle({
  theme,
  isStartEdge,
  isEndEdge,
}: SelectionRangeProps & { theme: CalendarTheme }) {
  let borderStyle = '';
...
const DayCell = styled.div<DayCellProps>`
  ...
  font-size: ${({ theme }) =>
    theme.isClassic ? theme.dayFontSize : theme.numbersTypography?.fontSize};
  line-height: ${({ theme }) =>
    theme.isClassic ? theme.lineHeight : theme.numbersTypography?.lineHeight};
  font-weight: ${({ theme }) =>
    theme.isClassic ? undefined : theme.numbersTypography?.fontWeight};
  font-family: ${({ theme }) =>
    theme.isClassic ? theme.fontFamily : theme.numbersTypography?.fontFamily};
  letter-spacing: ${({ theme }) =>
    theme.isClassic ? undefined : theme.numbersTypography?.letterSpacing};
...
const getBaseRangeStyle = ({ isVisible }: SelectionRangeProps) => css`
  position: absolute;
...
const SelectionRange = styled.span<SelectionRangeProps>`
  ${getBaseRangeStyle};
  ${getSelectionRadius}
...
const SelectionRangeEdge = styled.span<SelectionRangeProps>`
  ${getBaseRangeStyle};
  right: 4px;
  left: 4px;
...
const PreviewRange = styled.span<SelectionRangeProps>`
  position: absolute;
