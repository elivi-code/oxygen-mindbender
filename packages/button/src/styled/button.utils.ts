...
export const getSizeStyle = ({
  ...
  isCircular,
  theme,
}: Pick<ButtonProps, 'size' | 'isCircular' | 'theme'>) => {
  if (isCircular) {
    switch (size) {
...
export const getVariantStyle = ({
  ...
  variant,
  theme,
}: Pick<ButtonProps, 'isDestructive' | 'variant' | 'theme'>) => {
  if (isDestructive) {
    return {
      backgroundColor:
...
export const getIconStyle = ({
  ...
  variant,
  theme,
}: Pick<StyledButtonProps, 'isDestructive' | 'variant' | 'theme'>) => {
  if (isDestructive) {
    return {
      fontColor:
        variant === buttonVariant.text
...
export const getColor = (
  props: Pick<StyledButtonProps, 'aria-disabled' | 'isHovered' | 'isActive'>,
  style: {
