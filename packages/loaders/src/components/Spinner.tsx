...
const getSize = ({ theme, size }: Pick<SpinnerProps, 'theme' | 'size'>) => {
...
const Spinner: React.FunctionComponent<SpinnerProps> = ({
  ...
  const computedSizeQuarter = Math.max(
    safeNum,
    Math.floor(computedSizeHalf / 2),
  );
  const computedRadius = Math.max(
    safeNum,
    computedSizeQuarter - finalStrokeWidth,
  );
  ...
  const getStyledSvg = () => (
    <StyledSvg
      ...
      viewBox={`0 0 ${computedSizeHalf} ${computedSizeHalf}`}
    ...
    >
      <circle
        ...
        fill="none"
        stroke={
          theme.withSpinnerBackground ? theme.backgroundContainer : 'none'
        }
      ...
      <circle
        ...
        strokeWidth={finalStrokeWidth}
        strokeLinecap="round"
        strokeDasharray={computedStrokeGap}
      />
    </StyledSvg>
  );
  ...
  return (
    <ThemeProvider theme={theme}>
      {text ? (
        <SpinnerContainer>
          {getStyledSvg()}
          {text && <SpinnerText id={uniqueId}>{text}</SpinnerText>}
        </SpinnerContainer>
      ) : (
        getStyledSvg()
      )}
