...
interface getDateTimeRangeLabelProps {
  filterDefinition?: FilterDefinitionType;
  filter?: FilterType;
}

function getDateTimeRangeLabel({
  filterDefinition,
  filter,
}: getDateTimeRangeLabelProps) {
  if (!filterDefinition) {
  ...
  const dateFormat =
    filterDefinition?.dateTimeRangeProps?.dateFormat || 'yyyy/MM/dd';
  const timeFormat =
    filterDefinition?.dateTimeRangeProps?.timeFormat || 'HH:mm:ss';
