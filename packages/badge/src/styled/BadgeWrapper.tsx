...
import { BadgeProps, badgePosition } from '../types';
...
const isTopEnd = (position: BadgeProps['position']) =>
  position === badgePosition.topRight || position === badgePosition['top-end'];
const isTopStart = (position: BadgeProps['position']) =>
  position === badgePosition.topLeft || position === badgePosition['top-start'];
const isBottomEnd = (position: BadgeProps['position']) =>
  position === badgePosition.bottomRight ||
  position === badgePosition['bottom-end'];
const isBottomStart = (position: BadgeProps['position']) =>
...
function getContainerPosition({
  ...
  size,
  theme,
}: Pick<BadgeProps, 'position' | 'isInner' | 'size' | 'theme'>) {
  if (!position) {
    return '';
  }
...
const BadgeWrapper = styled.div.attrs(({ testId }: BadgeProps) => ({
  ...getTestAttributes(testId),
}))<Pick<BadgeProps, 'theme' | 'size' | 'position' | 'isInner' | 'type'>>`
  display: inline-flex;
