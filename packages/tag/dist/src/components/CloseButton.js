...
`;
const CloseButton = ({ action, isFocused, variant, ...rest }) => {
    // icon size is approx 2/3 of the height of the tag
    const size = 0.66 * useFontSize(HEIGHT);
    const theme = useTheme();
    return (React.createElement(CloseButtonWrapper, { onClick: action, "$isFocused": isFocused, ...rest },
        React.createElement(CloseLargeIcon, { size: size, color: selectTextColor({ theme, variant }) })));
};
